...
public class generateGameobjects : MonoBehaviour
{
    ...
    public GameObject[] block_celestials;
    ...
    void spawnobj_start(){ 
        ...
        //random generation planets of 3 diff sizes  initially
        int xObj = Random.Range(20, 25); // number of objects to be generated initially
        int xLarge = Random.Range(0, 2); // number of reward blocks to be generated initially
        ...
        while (i < xLarge)
        {
            ...
            initial_pos_x = Random.Range(-halfSize, halfSize) + transform.position.x; 
            initial_pos_y = Random.Range(-halfSize, halfSize) + transform.position.y; 
            bool isColliding = Physics.OverlapSphere(new Vector3(initial_pos_x, initial_pos_y, transform.position.z), 5)
                .Where(c => !c.gameObject.tag.Equals("chuck"))
                .ToArray().Length > 0;

            if (!isColliding) 
            {
                GameObject toSpawn = block_celestials[Random.Range(0, block_celestials.Length)];
