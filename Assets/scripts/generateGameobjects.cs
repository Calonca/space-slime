...
public class generateGameobjects : MonoBehaviour
{
    ...
    void spawnobj_start(){    //random generation planets of 3 diff sizes  initially
        ...
        while (i<x)
        {
            initial_pos_x = Random.Range(-14, 14); //range of X Coordinate to generate the planet object
            ...
            if (!Physics.CheckSphere(new Vector3(initial_pos_x, initial_pos_y, 0), 2)) // to check if any other sphere object(planet object ) generated within given pos and and radius
            {
                ...
                Instantiate(prefab_celestials[Random.Range(0, prefab_celestials.Length)], new Vector3(initial_pos_x, initial_pos_y, 0), rotation); //generate the planet of any of 3 diff size on the X and Y pos if above confition is false
                i++;
            }
    ...
    void spawnobj(){    //random generation planets of 3 diff sizes  initially
        ...
        while (i < x)
        {
            pos_x = Random.Range(-14,14); //range of X Coordinate to generate the planet object
            ...
            */
            ...
            {
                Quaternion rotation = Quaternion.Euler(0,0,Random.Range(0, 360));
                Instantiate(prefab_celestials[Random.Range(0, prefab_celestials.Length)], pos, rotation); //generate the planet of any of 3 diff size on the X and Y pos if above confition is false
                i++;
            }
